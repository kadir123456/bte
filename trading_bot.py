...
class TradingBot:
    ...
    def __init__(self, log_callback: Optional[Callable] = None, 
        ...
        """
        Botu başlatır ve gerekli ayarları yükler.
        
        Args:
            log_callback: Log mesajlarını arayüze göndermek için fonksiyon.
            ui_update_callback: Pozisyon/istatistik güncellemelerini arayüze göndermek için fonksiyon.
            status_callback: Botun çalışma durumunu (başladı/durdu) arayüze göndermek için fonksiyon.
        """
    ...
    def start_strategy(self):
        """Stratejiyi başlatır ve WebSocket dinleme döngüsünü çalıştırır."""
    ...
    def stop_strategy(self):
        """Çalışan WebSocket dinleyicilerini durdurur."""
    ...
    # --- 2. WebSocket Dinleyicileri ---

    async def listen_to_streams(self):
        """Binance'ten gelen mum ve kullanıcı verilerini eş zamanlı dinler."""
    ...
    async def _process_kline_message(self, msg: Dict[str, Any]):
        """Gelen mum verisini işler. Mum kapandığında stratejiyi tetikler."""
        if msg.get('e') == 'error':
            self._log(f"KLINE SOCKET HATASI: {msg.get('m')}")
            return

        if msg.get('k', {}).get('x'): # Mum kapandı mı?
    ...
    async def _process_user_message(self, msg: Dict[str, Any]):
        """Kullanıcı verilerini (emir güncellemeleri, hesap değişiklikleri) işler."""
        ...
        if event_type == 'ACCOUNT_UPDATE':
            ...
            if order_status in ['FILLED', 'CANCELED', 'EXPIRED']:
                ...
                # Sadece pozisyon kapatan (kâr/zarar realize eden) işlemleri kaydet
                if float(order_data.get('rp', 0)) != 0:
                    self._log(f"POZİSYON KAPANDI: {order_data.get('s')} | PNL: {order_data.get('rp')} USDT")
                    # Gelen veriyi veritabanı fonksiyonunun beklediği formata çevir
                    trade_to_log = {
                ...
                # Her emir güncellemesinde arayüzü yenile
                if self.ui_update_callback: self.ui_update_callback()
    
    # --- 3. Ana İşlem Fonksiyonları ---

    def _open_position(self, side: str, atr: float):
        """Verilen yönde yeni bir pozisyon açar ve TP/SL emirlerini yerleştirir."""
        try:
            ...
            time.sleep(1) # Siparişin dolması ve pozisyonun oluşması için bekle
    ...
    def _set_tp_sl(self, side: str, atr: float):
        """Bir pozisyon için Take Profit ve Stop Loss emirlerini ayarlar."""
        try:
    ...
    def _close_position_and_log(self, reason: str):
        """Mevcut pozisyonu piyasa emriyle kapatır ve açık emirleri iptal eder."""
        try:
            position = self.get_position_info(self.active_symbol)
            if not position:
            ...
            # Önce açık emirleri iptal et, sonra pozisyonu kapat
    ...
    # --- 4. Manuel Kontrol ve Ayar Fonksiyonları ---
    ...
    def update_active_symbol(self, new_symbol: str):
        """İşlem yapılacak sembolü günceller. Strateji çalışıyorsa yeniden başlatır."""
    ...
    # --- 5. Veri Getirme ve Yardımcı Fonksiyonlar ---
    ...
    def get_current_position_data(self) -> Optional[dict]:
        try:
            ...
            return {
                "symbol": position.get('symbol'), "quantity": position_amt, "entry_price": f"{entry_price:.5f}",
    ...
    def get_all_usdt_symbols(self) -> List[str]:
        try:
            ...
            self._log(f"API HATASI: Sembol listesi çekilemedi: {e}"); return []
